import { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import Table from '../../../components/tables/Table';
import axios from 'axios';
import DeleteModal from '../../../components/utils/DeleteModal';
import CreateButton from '../../../components/utils/CreateButton';


const Carousel = () => {
    const BASE_URL = import.meta.env.VITE_BASE_URL;
    const [loading, setLoading] = useState<boolean>(true);
    const [error, setError] = useState<string | null>(null);
    const [carousels, setCarousel] = useState<Carousel[]>([]);
    const [showDeleteModal, setShowDeleteModal] = useState(false);
    const [selectedCarousel, setSelectedCarousel] = useState<Carousel | null>(null);
    const [pagination, setPagination] = useState({
        page: 1,
        limit: 10,
        total: 0
    });
    const navigate = useNavigate();

    const fetchData = async (page: number, limit: number) => {
        try {
            setLoading(true);
            const response = await axios.get(`${BASE_URL}/carousel`, {
                params: {
                    page,
                    limit
                }
            });
            const { data, pagination: pageMeta } = response.data;

            setCarousel(data);
            setPagination({
                page: pageMeta.currentPage,
                limit: pageMeta.limit,
                total: pageMeta.totalItems
            });
            setError(null);
        } catch (err) {
            setError('Failed to fetch data. Please try again later.');
            console.error('Error fetching data:', err);
        } finally {
            setLoading(false);
        }
    };

    useEffect(() => {
        fetchData(pagination.page, pagination.limit);
    }, [pagination.page, pagination.limit]);

    const handleUpdate = (carousel: Carousel) => {
        navigate(`/cms/carousels/${carousel._id}/update`, {
            state: { carousel } // Passing carousel data via state
        });
    };

    const handleDelete = (carousel: Carousel) => {
        setSelectedCarousel(carousel);
        setShowDeleteModal(true);
    };
    const confirmDelete = async (carousel: Carousel) => {
        try {
            await axios.delete(`${BASE_URL}/carousel/${carousel._id}`);
            setCarousel(carousels.filter(p => p._id !== carousel._id));
            setShowDeleteModal(false);
            setSelectedCarousel(null);
        } catch (err) {
            console.error('Error deleting product:', err);
            alert('Failed to delete product. Please try again.');
        }
    };
    const handleView = (carousel: Carousel) => {
        navigate(`/cms/carousels/${carousel._id}/detail`, {
            state: { carousel } // Passing product data via state
        });
    };
    const handleImageView = (data: Carousel) => {
        navigate(`/cms/images`, { state: { refId: data._id, refModel: 'Carousel' } })

    }
    const onPageChange = async (page: number) => {
        setPagination(prev => ({
            ...prev,
            page
        }));
    };

    const onLimitChange = async (limit: number) => {
        setPagination(prev => ({
            ...prev,
            limit,
            page: 1 // Reset to first page when changing limit
        }));
    };

    if (loading) {
        return <div className="p-4">Loading data...</div>;
    }

    if (error) {
        return <div className="p-4 text-red-600">{error}</div>;
    }
    return (
        <div className="w-full max-w-full overflow-x-auto">
            <Table
                title="Carousels"
                headers={[
                    { key: 'title', label: 'Title', sortable: true },
                    { key: 'description', label: 'Description', sortable: true },
                    { key: 'buttonText', label: 'Button Text', sortable: true },
                    { key: 'order', label: 'Order', sortable: true },
                    {
                        key: 'Active',
                        label: 'Active',
                        sortable: true,
                        render: (value) => (
                            <span className={`px-2 py-1 rounded-full text-xs ${value === true ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                }`}>
                                {value === true ? 'Yes' : 'No'}
                            </span>
                        )
                    }
                ]}
                data={carousels}
                keyField="_id"
                selectable
                onRowSelect={(selected) => console.log('Selected rows:', selected)}
                // Enable action buttons
                createBtn={<CreateButton link='/cms/carousels/create' />}
                showActions={true}
                onUpdate={handleUpdate}
                onDelete={handleDelete}
                onView={handleView}
                onImages={handleImageView}
                actionLabels={{ update: 'Edit', delete: 'Remove', view: 'View', images: 'Images' }}
                pagination={{
                    page: pagination.page,
                    limit: pagination.limit,
                    total: pagination.total,
                    onPageChange: onPageChange,
                    onLimitChange: onLimitChange
                }}
            />

            <DeleteModal
                data={selectedCarousel}
                show={showDeleteModal}
                title="Delete Carousel"
                onClose={() => setShowDeleteModal(false)}
                onConfirmDelete={confirmDelete}
            />
        </div>

    );

}
export default Carousel