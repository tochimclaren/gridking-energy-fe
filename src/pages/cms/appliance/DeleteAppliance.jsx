...
const DeleteAppliance = () => {
  ...
  useEffect(() => {
    // If no appliance data was passed, redirect to appliances list
    if (!appliance) {
      navigate('/cms/appliances');
    }
  }, [appliance, navigate]);
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`/appliance/${appliance._id}`);
      
      // Show brief success message then redirect
      setTimeout(() => {
        navigate('/cms/appliances', { 
          state: { message: `${appliance.name} has been deleted successfully.` } 
        });
      }, 500);
      ...
      setStatus({
        loading: false,
        error: error.response?.data?.message || 'An error occurred while deleting the appliance.',
      });
    }
  };
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-lg">
      <div className="text-center">
        ...
        <h3 className="mt-5 text-xl font-bold text-gray-900">Delete Appliance</h3>
        
        <div className="mt-4 mb-6">
          <p className="text-gray-600">
            Are you sure you want to delete <span className="font-semibold text-gray-800">"{appliance.name}"</span>?
          </p>
          <p className="mt-2 text-sm text-gray-500">
            This action cannot be undone. This will permanently delete the appliance from the database.
          </p>
        </div>
