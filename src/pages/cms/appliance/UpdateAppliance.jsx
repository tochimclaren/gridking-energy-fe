...
const UpdateAppliance = () => {
  ...
  useEffect(() => {
    ...
    // Initialize form with appliance data
    setFormData({
      name: appliance.name || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`/appliance/${appliance._id}`, {
        name: formData.name,
      ...
      setStatus({
        loading: false,
        error: error.response?.data?.message || 'An error occurred while updating the appliance.',
        success: ''
      });
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Update Appliance</h2>
        <button
